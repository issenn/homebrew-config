...
homebrew_bundle_file.dirname.glob(FIND_TAP_WILDCARD_PATTERN).reject{ |f| f.directory? || IGNORES.include?(f.basename.to_s) || f.to_s.match?(ignores_tap_re) }.each do |brewfile|
  puts "Tap Brewfile: #{brewfile}" if args.debug? || args.verbose?
  eval(brewfile.realpath.read, binding) if brewfile.realpath.exist?
end

homebrew_bundle_file.dirname.glob(FIND_MAS_WILDCARD_PATTERN).reject{ |f| f.directory? || IGNORES.include?(f.basename.to_s) || f.to_s.match?(ignores_mas_re) }.each do |brewfile|
  puts "Mas Brewfile: #{brewfile}" if args.debug? || args.verbose?
  eval(brewfile.realpath.read, binding) if brewfile.realpath.exist?
end

homebrew_bundle_file.dirname.glob(FIND_WILDCARD_PATTERN).reject{ |f| f.directory? || IGNORES.include?(f.basename.to_s) || f.to_s.match?(ignores_re) }.each do |brewfile|
